...
// Configure multer storage to create a unique folder per user
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
...
router.post('/', authMiddleware, (req, res) => {
  upload(req, res, async (err) => {
    if (err) return res.status(400).json({ message: 'Error uploading images: ' + err.message });
...
// Get details of a specific car for the authenticated user
router.get('/:id', authMiddleware, async (req, res) => {
  try {
...
// Update car details and images for the authenticated user
router.put('/:id', authMiddleware, (req, res) => {
  upload(req, res, async (err) => {
    if (err) return res.status(400).json({ message: 'Error uploading images: ' + err.message });
    ...
    try {
      ...
      // Filter out deleted images from car's existing images
      car.images = car.images.filter(image => !deletedImages.includes(image));
...
// Delete a car and its images for the authenticated user
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    const car = await Car.findOne({ _id: req.params.id, userId: req.userId });
    if (!car) return res.status(404).json({ message: 'Car not found or unauthorized' });

    // Delete each image from the user's folder
    car.images.forEach(imagePath => {
      const filePath = path.join(__dirname, '..', imagePath);
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
    });
